...
const string declkeypow = "pow";
const string declkeysqrt = "sqrt";
	...
	switch (ch)
	{
    	...
    	default: 
    	{
    		if (isalpha(ch))
    		{
    			...
    			else if (s == declkeypow) return Token{power};
    			else if (s == declkeysqrt) return Token{square};
...
double sqrt2() 
{
	Token t = ts.get();
	        if (t.kind != '(') error("'(' expected");
	        double d=expression();
	        if (d<0) error("negative numbers haven't got square roots.");
	Token t2 = ts.get();
